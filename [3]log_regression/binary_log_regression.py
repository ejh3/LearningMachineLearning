...
class BinaryLogReg:
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        J = (np.log((1 + np.exp(-y*(self.bias + X @ self.weight))))).mean()
        J = J + self._lambda * np.sum(self.weight**2)
        return J
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        n, d = X.shape
        mu = self.mu(X, y)
        grad = (-X.T @ ((1 - mu) * y) / n + 2 * self._lambda * self.weight).squeeze()
        assert grad.shape == (d,)
        return grad
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        n, d = X.shape
        y_pred = np.sign(X @ self.weight + self.bias).squeeze()
    ...
    def train(
        ...
        for i in range(epochs):
            ...
            # Train over all (well, most depending on batch size) data
            for j in range(num_batches):
                start = j * batch_size
                stop = (j + 1) * batch_size
                self.step(X_shuffled[start:stop], y_shuffled[start:stop], learning_rate)
            ...
            print(
                f"Epoch {i+1}/{epochs}, Train loss: {train_loss:.4f}, Train error: \
                 {train_error:.4f}, Test loss: {test_loss:.4f}, Test error: {test_error:.4f}"
            )
        return result
...
if __name__ == "__main__":
    ...
    history = model.train(x_train, y_train, x_test, y_test, batch_size=1, learning_rate=1e-4)
